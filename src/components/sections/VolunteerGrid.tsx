    ...
    <div className="mb-16">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {volunteers.map((v, idx) => {
          if (v.isPlaceholder) {
            return (
              <div key={`${v.location}-${idx}`} className="flex flex-col items-center justify-center rounded-xl bg-brand-sand-light p-6 shadow-sm border-2 border-dashed border-brand-teal/30">
                <div className="text-center">
                  <p className="text-base font-bold text-brand-teal mb-2">Looking for Leader</p>
                  <p className="text-xs text-gray-600 mb-3">{v.location}</p>
                  <Link
                    href="/contact"
                    className="inline-flex items-center text-sm font-medium text-brand-green hover:text-brand-green-light transition-colors"
                  >
                    Contact us
